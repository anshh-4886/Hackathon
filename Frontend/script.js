/* ===================================================
   TrustChain Local – Single script for:
   - Landing modal (Get Started / Sign In)
   - Demo dashboard auto login
   - App pages + mini modal
   - ✅ Added: REAL auth (backend) + DEMO isolation
=================================================== */

const DB = {
  // ✅ DEMO storage (isolated)
  ...
  // ✅ REAL auth storage
  token: "tc_token",
  mode: "tc_mode", // "demo" | "real"

  // (kept for backward compatibility; not used for demo anymore)
  user: "tc_user",
...
/* ===================================================
   ✅ "ACTIVE DB KEYS" (ADDED)
   Demo uses demo_* keys
   Real mode fetches from backend (no local storage for data)
=================================================== */
function activeKeys() {
  if (isDemo()) {
    return {
...
function getUser() {
  ...
  // real mode: user comes from token; but keep minimal fallback
...
function protect() {
  ...
  // ✅ Demo always allowed (auto create)
  if (isDemo()) {
    ensureDemoUser();
    return;
  }
...
function logout() {
  // ✅ Do not delete demo data, only end session
  clearToken();
  setMode("demo");
  // also remove any old "real local" fallback user
  localStorage.removeItem(DB.user);
  window.location.href = "index.html";
}

/* ---------- Seed demo data ---------- */
/* (Your original seed() kept, but now it writes DEMO keys only) */
function seed() {
  const K = activeKeys();

  // ✅ If real mode, do nothing (real data comes from backend)
  if (isReal()) return;
...
/* ---------- Demo login (for Open Demo Dashboard) ---------- */
...
function wireLandingModal() {
  ...
  // ✅ Make "Open Demo Dashboard" always work (DEMO mode)
  ...
  /* ===================================================
     ✅ REAL SIGNUP (Backend) — Added
     No demo overwrite now.
  =================================================== */
  ...
  if (signupForm) {
    signupForm.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        // optional store some real user display
...
/* ===================================================
   ✅ REAL DATA FETCH HELPERS (ADDED)
=================================================== */
async function loadRealDataIfNeeded() {
  if (!isReal()) return;

  // if token missing, kick to index
  if (!getToken()) {
    window.location.href = "index.html";
    return;
  }
  ...
  try {
    ...
    // if backend not reachable, fallback to demo mode
    setMode("demo");
    ensureDemoUser();
  }
}
...
function renderLedger() {
  ...
  if (sales.length === 0) {
    ...
    sales.slice().reverse().forEach(sale => {
      ...
      // DEMO: pending UPI click to confirm
...
/* (Your duplicate showAddSale + saveSale kept as-is below — not deleting) */
function showAddSale() {
  openMiniModal("Add Sale", `
    <form id="addSaleForm" class="mini-form">
      <div class="form-row">
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // ✅ if app page and real mode, load DB data first
  if (isAppPage() && isReal()) {
    await loadRealDataIfNeeded();
  }
